...
New code should prefer:
    ...
    eye_events, samples = load("UNL_UM", sample_size=100)
...
def UNL_UM(sample_size: int = None):
    """Parse the UNL_UM dataset.

    Parameters
    ----------
    sample_size : int, optional
        Number of participant TSV files to read. If None, the default
        from `datasets_config.DATASETS["UNL_UM"]["default_sample_size"]`
        is used (44, preserving the legacy default).
    ...
    if sample_size is None:
        sample_size = DATASETS["UNL_UM"]["default_sample_size"]
    return _load("UNL_UM", sample_size=sample_size)
